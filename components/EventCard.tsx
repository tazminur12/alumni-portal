...
export default function EventCard({ event, user, isRegistered = false, onRegistrationSuccess }: EventCardProps) {
  ...
  return (
    <>
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm transition-shadow hover:shadow-md flex flex-col">
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className={`rounded-full px-3 py-1 text-xs font-semibold ${event.status === "upcoming" ? "bg-green-50 text-green-600" : "bg-gray-100 text-gray-600"}`}>
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted grow">
          {event.description}
        </p>
        ...
        {canRegister && (
          <button
            ...
            className="mt-6 w-full rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-primary-dark"
          >
            Register for Event
          </button>
        )}
        {isRegistered && (
          <div className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-green-50 px-4 py-2.5 text-sm font-semibold text-green-700">
            <CheckCircle size={18} />
            Already Registered
          </div>
        )}
